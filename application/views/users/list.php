          ...
          <div class="box">
                <div class="box-header">               
				  <div class="btn-group pull-right">
					    <?php if(isset($access[2][1])){?>
							<a href="<?php echo base_url('users/add');?>" class="btn btn-success"><i class="fa fa-plus"></i>&nbsp;Dodaj</a>
						<?php }?>
						<?php if(isset($access[2][2])){?>
							<button type="button" id="buttonEdit" class="btn btn-default" disabled><i class="fa fa-pencil"></i>&nbsp;Edytuj</button>
						<?php }?>
						<?php if(isset($access[2][3])){?>
...
<div style="display:none">
 <?php echo form_open(base_url("users/edit"),array('id' => 'editFormSend','method' => 'post'),array());?>
	<input type="hidden" name="id_edit" id="id_edit" class="form-control" />
	<input type="hidden" name="redirect"  class="form-control" value='1' />
  <?php echo form_close();?>
  <?php echo form_open(base_url("users/delete"),array('id' => 'deleteFormSend','method' => 'post'),array());?>
...
<script>
    $(function () {
        dTable = $("#dataTable").DataTable( {
			...
			"serverSide": true,
			"order": [[ 1, "asc" ]],
			"ajax": "<?php echo base_url("users/getAllTable");?>",
			"language": {
